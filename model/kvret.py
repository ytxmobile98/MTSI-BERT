...
class KvretDataset(Dataset):
    ...
    def __init__(self, json_path):
        ...
        def get_action(items): return 'insert' if items is None else 'fetch'
    ...
    def get_action_frequency(self):
        ...
        assert fetch_count + \
            insert_count == len(
                self._dataset), '[ASSERT FAILED] -- Misleading count!'
    ...
    def remove_subsequent_actor_utterances(self):
        ...
        for dialogue in self._dataset:
            ...
            assert len(dialogue['utterances']) == len(
                dialogue['turns']), '[ASSERT FAILED] -- len(utt) != len(turns)'
