...
class TwoSepTensorBuilder(MTSITensorBuilder):
    ...
    def build_tensor(self, windows_list, device):
        ...
        sep_tensor = torch.tensor(
            MTSITensorBuilder._BERT_SEP_IDX).reshape(1).to(device)
        max_win_len = 0

        for win in windows_list:
            # at first append [CLS]
            curr_builded_t = torch.tensor(
                MTSITensorBuilder._BERT_CLS_IDX).reshape(1).to(device)
            for idx, t in enumerate(win):
                ...
                if idx == len(win) - 1:
                    ...
                    if idx == 0:
                        curr_builded_t = torch.cat(
                            (curr_builded_t, no_padded, sep_tensor))
    ...
    def build_attention_and_segment(self, bert_input):
        ...
        for idx, t in enumerate(bert_input):
            ...
            if idx != 0 and t[0] != 0:
                first_segment_end = self.__find_first_occurrence(
                    t, MTSITensorBuilder._BERT_SEP_IDX)
